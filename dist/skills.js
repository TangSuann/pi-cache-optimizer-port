...
export function formatSkillsForPromptCompressed(skills) {
    const visible = visibleSkills(skills);
    if (visible.length === 0)
        return "";
 ...
 */
export function compressSkillsInSystemPrompt(prompt, skills) {
    ...
    if (!compressed || compressed.length >= verbose.length)
        return prompt;
